...
def perform_cpa(keys, traces, challenges, dist, plot, correlations):
    ...
    for kguess in tqdm(keys, desc="Key guess loop"):
        ...
        for tnum in range(0, numtraces):
            ...
            if plot and (tnum >= THREAD_START and tnum < THREAD_END):
                cpaoutput_tmp = sumnum / np.sqrt(sumden1 * sumden2)
                maxcpa_tmp = max(abs(cpaoutput_tmp))

                correlations.setdefault(kguess, []).append(maxcpa_tmp)
...
def recover_mantissa(traces, challenges, verbose, plot, correlation):
    ...
    guess = perform_cpa(keys, traces, challenges, intermediate, True, correlation)
...
def recover_exponent(keys, traces, challenges, verbose, plot):
    def intermediate(op, key):
    ...
    guess = perform_cpa(keys, traces, challenges, intermediate, False, {})
    if verbose:
...
def convert_to_serializable(obj):
    if isinstance(obj, np.ndarray):
        ...
        return [
            convert_to_serializable(item) for item in obj
        ...
        return obj  # Return as-is for non-numpy and non-iterable objects
...
def run_sca(nb_tests, nb_traces, file, verbose, plot, all_data, detect_error):
    ...
    for t in range(nb_tests):
        ...
        offset = 103  # this offset has to be set manually
        points_per_mul = 450  # this has to be set manually
        period = 1390
        # pattern = np.mean([t[offset:offset+points_per_mul] for t in traces], axis=0)

        # indexes = find_all_indexes(traces, pattern, DIM, True)
        # print(indexes)
        indexes = [
            5,
            2785,
        ...
        partial_traces = np.array(
            [t[indexes[4 * i] : indexes[4 * i] + points_per_mul] for t in traces]
        )
        partial_challenges = np.array([row[i] for row in challenges])

        if all_data:  # add 4th mul pattern to the 1st
            partial_traces = np.concatenate(
                [
                    partial_traces,
                    [
                        t[indexes[4 * i + 3] : indexes[4 * i + 3] + points_per_mul]
                        for t in traces
                    ],
                ]
            )
            partial_challenges = np.concatenate(
                [
                    partial_challenges,
                    [row[HALF + i] for row in challenges],
                ]
            )

        # new_traces, new_challenges = remove_noise(partial_traces, partial_challenges)
        new_traces, new_challenges = partial_traces, partial_challenges
        recovered = recover_mantissa(
            new_traces, new_challenges, verbose, plot, correlation
        )
        # Convert the correlation dictionary
        D_serializable = {k: convert_to_serializable(v) for k, v in correlation.items()}
        # Store the dictionary in a file
        with open(
            f"./correlation/cw_correlation.json",
            "w",
        ) as f:
            json.dump(D_serializable, f)
        correlation = {}
        recovered = recover_exponent(
            recovered, new_traces, new_challenges, verbose, plot
        )
        guess[i] = recover_sign(
            recovered, partial_traces, partial_challenges, False, plot
        )
        if verbose:
        ...
        partial_traces = np.array(
            [
                t[indexes[4 * i + 1] : indexes[4 * i + 1] + points_per_mul]
                for t in traces
            ]
        )
        ...
        if all_data:  # add 3rd mul pattern to the second
            partial_traces = np.concatenate(
                [
                    partial_traces,
                    [
                        t[indexes[4 * i + 2] : indexes[4 * i + 2] + points_per_mul]
                        for t in traces
                    ],
                ]
            )
            partial_challenges = np.concatenate(
                [partial_challenges, [row[i] for row in challenges]]
            )

        new_traces, new_challenges = remove_noise(partial_traces, partial_challenges)
        recovered = recover_mantissa(new_traces, new_challenges, False, plot, {})
        recovered = recover_exponent(
            recovered, new_traces, new_challenges, verbose, plot
        )
        guess[HALF + i] = recover_sign(
            recovered, partial_traces, partial_challenges, verbose, plot
        )
        if verbose:
        ...
        if detect_error:
            ...
            partial_challenges = np.array([row[i] for row in challenges])
            recovered = recover_mantissa(
                partial_traces, partial_challenges, verbose, plot, {}
            )
            recovered = recover_exponent(
                recovered, partial_traces, partial_challenges, verbose, plot
            )
            second_guess[HALF + i] = recover_sign(
                recovered, partial_traces, partial_challenges, verbose, plot
            )
            if verbose:
                print(
                    "second guess: {}\tkey: {}".format(
                        second_guess[HALF + i], key_fft[HALF + i]
                    )
                )
            ...
            second_guess[i] = recover_sign(
                recovered, partial_traces, partial_challenges, verbose, plot
            )
            if verbose:
                print("second guess: {}\tkey: {}".format(second_guess[i], key_fft[i]))

        s_, e_, m_ = decompose(key_fft[0])
                ...
                if guess[i] != second_guess[i]:
                    print(
                        "mismatch at index {}: {} {} {}".format(
                            i, guess[i], second_guess[i], key_fft[i]
                        )
                    )

        print("key fft:\t", key_fft)
        print("guess fft:\t", guess)
        iFFT(guess, LOGN)
...
def usage(exe):
    ...
    print("Options:")
    for elem in opt:
        print(" {0:30} {1}".format(elem, opt[elem]))
    ...
    if len(argv) > 0:
        try:
            opts, args = getopt.getopt(
                argv,
                "vpha",
                ["nb-tests=", "nb-traces=", "input-file=", "help", "all", "detect"],
            )
