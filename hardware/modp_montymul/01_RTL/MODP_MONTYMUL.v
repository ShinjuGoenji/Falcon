...
assign w_q = a_b_p0i[Q_WIDTH-1:0] * p;
assign w_p = a_b_p0i[P_WIDTH-1:0] * p;
// assign w_q = a_b_p0i_reg[Q_WIDTH-1:0] * p_reg;
// assign w_p = a_b_p0i_reg[P_WIDTH-1:0] * p_reg;

assign z_w_q = a_b + w_q;
assign z_w_p = a_b + w_p;
...
always @(*) begin
    if (isMQ) begin
        if (z_w_q_shift >= p)
            d_comb = z_w_q_shift - p;
        else
            d_comb = z_w_q_shift;
    end
    else begin
        if (z_w_p_shift >= p)
            d_comb = z_w_p_shift - p;
        else
            d_comb = z_w_p_shift;
    end
end
