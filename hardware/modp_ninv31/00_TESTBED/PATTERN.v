...
task reset_task; begin 
    ...
    if(p0i !== 'b0) begin 
        $display("************************************************************");  
        $display("                          FAIL!                             ");    
        $display("  	'p0i' should be 0 after initial RESET  at %8t   	  ",$time);
        $display("************************************************************");
        repeat(2) #CYCLE;
        $finish;
    end
...
task input_task;
	reg [P_WIDTH-1:0] p_pat;
	begin
	fscanf_int = $fscanf(file_in, "%d", p_pat);
	in_valid = 'b1;
	p = p_pat;
	for (i_stage = 0; i_stage < PIPLINE_STAGES; i_stage = i_stage + 1)
...
task check_ans_task; 
	...
	if(out_valid === 1) begin
		fscanf_int = $fscanf(file_out, "%d", p0i_pat);
		if(p0i !== p0i_pat)begin
            $display("***********************************************************");     
            $display("                          FAIL!                          	 ");  
